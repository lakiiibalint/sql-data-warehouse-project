...
| `country` | NVARCHAR(50)| Country of residence. Used for geographic segmentation and reporting. |
| `marital_status` | NVARCHAR(50)| Marital status of the customer (e.g. Single, Married). |
| `gender` | NVARCHAR(50)| Gender of the customer. |
| `birthdate` | DATE| Date of birth. Can be used to derive age and age group metrics. |
...
| `product_number` | NVARCHAR(50)| Human-readable product reference code (e.g. SKU or part number). |
...
| `subcategory` | NVARCHAR(50)| Sub-classification within the category (e.g. Road Bikes, Helmets). |
| `maintenance` | NVARCHAR(50)| Indicates maintenance requirements or classification for the product. |
| `cost` | INT| Standard cost of the product. Used in margin and profitability calculations. |
...
| `order_date` | DATE| Date the order was placed. Primary date dimension for time-series analysis. |
| `shipping_date` | DATE| Date the order was shipped. Used to compute fulfilment lead time. |
| `due_date` | DATE| Expected delivery due date. Used for SLA and on-time delivery analysis. |
| `sales_amount` | INT| Total sales value for the line item (quantity × price, or pre-calculated). |
